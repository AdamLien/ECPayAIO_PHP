...
{	
	...
	*/
	...
	{		
		...
		else
		{
			...
			// A.一般開立發票 
			if( $this->Invoice_Method == EcpayInvoiceMethod::INVOICE )
			{
				// 3-1過濾不需要的項目
			...
			// B.延遲或觸發開立發票
			if( $this->Invoice_Method == EcpayInvoiceMethod::INVOICE_DELAY )
			{
				// 3-1過濾不需要的項目
			...
			// C.開立折讓
			if( $this->Invoice_Method == EcpayInvoiceMethod::ALLOWANCE )
			{
				// 3-1過濾不需要的項目
			...
			// D.發票作廢
			if( $this->Invoice_Method == EcpayInvoiceMethod::INVOICE_VOID )
			{
				// 3-1過濾不需要的項目
				unset($aSend_Info['RelateNumber']) ;
				unset($aSend_Info['CustomerID']) ;
			...
			// E.折讓作廢
			if( $this->Invoice_Method == EcpayInvoiceMethod::ALLOWANCE_VOID )
			{
				// 3-1過濾不需要的項目
				unset($aSend_Info['RelateNumber']) ;
				unset($aSend_Info['CustomerID']) ;
			...
			// F.查詢發票 G.查詢作廢發票
			if( $this->Invoice_Method == EcpayInvoiceMethod::INVOICE_SEARCH || $this->Invoice_Method == EcpayInvoiceMethod::INVOICE_VOID_SEARCH )
			{
				// 3-1過濾不需要的項目
				unset($aSend_Info['CustomerID']) ;
				unset($aSend_Info['CustomerIdentifier']) ;
			...
			// H.查詢折讓明細 I.查詢折讓作廢明細
			if( $this->Invoice_Method == EcpayInvoiceMethod::ALLOWANCE_SEARCH || $this->Invoice_Method == EcpayInvoiceMethod::ALLOWANCE_VOID_SEARCH )
			{
				// 3-1過濾不需要的項目
				unset($aSend_Info['RelateNumber']) ;
				unset($aSend_Info['CustomerID']) ;
			...
			// J.發送通知
			if( $this->Invoice_Method == EcpayInvoiceMethod::INVOICE_NOTIFY )
			{
				// 3-1過濾不需要的項目
				unset($aSend_Info['RelateNumber']) ;
				unset($aSend_Info['CustomerID']) ;
			...
			// K.查詢折讓明細
			if( $this->Invoice_Method == EcpayInvoiceMethod::INVOICE_TRIGGER )
			{
				// 3-1過濾不需要的項目
				unset($aSend_Info['RelateNumber']) ;
				unset($aSend_Info['CustomerID']) ;
			...
			// L.手機條碼驗證
			if( $this->Invoice_Method == EcpayInvoiceMethod::CHECK_MOBILE_BARCODE )
			{
				// 3-1過濾不需要的項目
				unset($aSend_Info['RelateNumber']) ;
				unset($aSend_Info['CustomerID']) ;
			...
			// M.愛心碼驗證
			if( $this->Invoice_Method == EcpayInvoiceMethod::CHECK_LOVE_CODE )
			{
				// 3-1過濾不需要的項目
				unset($aSend_Info['RelateNumber']) ;
				unset($aSend_Info['CustomerID']) ;
			...
			{
		  		// A. B. C. D. E.
		  		if( $this->Invoice_Method == EcpayInvoiceMethod::INVOICE || $this->Invoice_Method == EcpayInvoiceMethod::INVOICE_DELAY || $this->Invoice_Method == EcpayInvoiceMethod::ALLOWANCE || $this->Invoice_Method == EcpayInvoiceMethod::INVOICE_VOID || $this->Invoice_Method == EcpayInvoiceMethod::ALLOWANCE_VOID)
	  			{
					// 5-1 CheckMac檢查
				...
				// F.查詢發票
				if( $this->Invoice_Method == EcpayInvoiceMethod::INVOICE_SEARCH )
	  			{
	  				// 5-1 CheckMac檢查
					if(count($aReturn_Info) > 0 && isset($aReturn_Info['CheckMacValue']))
					{
	  			...
	  			// G.查詢作廢發票
				if( $this->Invoice_Method == EcpayInvoiceMethod::INVOICE_VOID_SEARCH )
	  			{
	  				// 5-1 CheckMac檢查
					if(count($aReturn_Info) > 0 && isset($aReturn_Info['CheckMacValue']))
					{
	  			...
	  			// H.查詢折讓明細
				if( $this->Invoice_Method == EcpayInvoiceMethod::ALLOWANCE_SEARCH )
	  			{
	  			...
	  			// I.查詢折讓作廢明細
				if( $this->Invoice_Method == EcpayInvoiceMethod::ALLOWANCE_VOID_SEARCH )
	  			{
	  				// 5-1 CheckMac檢查
					if(count($aReturn_Info) > 0 && isset($aReturn_Info['CheckMacValue']))
					{
	...
	*/
	        ...
	        if(strlen($this->Invoice_Url) == 0)
		{
	        ...
	        if( $this->Invoice_Method == EcpayInvoiceMethod::INVOICE || $this->Invoice_Method == EcpayInvoiceMethod::INVOICE_DELAY || $this->Invoice_Method == EcpayInvoiceMethod::INVOICE_SEARCH || $this->Invoice_Method == EcpayInvoiceMethod::INVOICE_VOID_SEARCH)
	        {	
	        	// 4.廠商自訂編號
	        ...
	        if( $this->Invoice_Method == EcpayInvoiceMethod::INVOICE || $this->Invoice_Method == EcpayInvoiceMethod::INVOICE_DELAY )
	        {	
	        	// 5.客戶代號 CustomerID
		...
		// 檢查 A.一般開立發票、B.延遲觸發開立發票
	        ...
	        {		        	
	        	...
	        	if ($this->Send['Print'] == EcpayPrintMark::Yes)
        		{
			...
			if ($this->Send['TaxType'] == EcpayTaxType::Zero)
			{
				if ( ( $this->Send['ClearanceMark'] != EcpayClearanceMark::Yes ) && ( $this->Send['ClearanceMark'] != EcpayClearanceMark::No ) )
				{
			...
			else
			{
	        	...
	        	// *列印註記僅能為 0 或 1
			if ( ( $this->Send['Print'] != EcpayPrintMark::Yes ) && ( $this->Send['Print'] != EcpayPrintMark::No ) )
			{
				array_push($arErrors, "12:Invalid Print.");
			}
			// *若捐贈註記 = '1' (捐贈)時，則VAL = '0' (不列印)
			if ($this->Send['Donation'] == EcpayDonation::Yes)
			{
				if ($this->Send['Print'] != EcpayPrintMark::No)
				{
			...
			if ( ($this->Send['Donation'] != EcpayDonation::Yes ) && ( $this->Send['Donation'] != EcpayDonation::No ) )
			{
				array_push($arErrors, "13:Invalid Donation.");
			}
			// *若統一編號有值時，則VAL = '2' (不捐贈)
			if (strlen($this->Send['CustomerIdentifier']) > 0 && $this->Send['Donation'] == EcpayDonation::Yes )
			{
			...
			if ( ( $this->Send['CarruerType'] != EcpayCarruerType::None ) && ( $this->Send['CarruerType'] != EcpayCarruerType::Member ) && ( $this->Send['CarruerType'] != EcpayCarruerType::Citizen ) && ( $this->Send['CarruerType'] != EcpayCarruerType::Cellphone ) )
			{
			...
			else
			{
				...
				if (strlen($this->Send['CustomerIdentifier']) > 0)
				{
					if ($this->Send['CarruerType'] == EcpayCarruerType::Member || $this->Send['CarruerType'] == EcpayCarruerType::Citizen )
					{
                          			array_push($arErrors, "15:Invalid CarruerType.");
                      			}		
				}
			...
			if ( ( $this->Send['TaxType'] != EcpayTaxType::Dutiable ) && ( $this->Send['TaxType'] != EcpayTaxType::Zero ) && ( $this->Send['TaxType'] != EcpayTaxType::Free ) && ( $this->Send['TaxType'] != EcpayTaxType::Mix ) )
			{
		...
		// 檢查 A.一般開立發票、B.延遲觸發開立發票、C.開立折讓
	        if( $this->Invoice_Method == EcpayInvoiceMethod::INVOICE || $this->Invoice_Method == EcpayInvoiceMethod::INVOICE_DELAY || $this->Invoice_Method == EcpayInvoiceMethod::ALLOWANCE )
	        {	
		...
		// 檢查 A.一般開立發票、B.延遲觸發開立發票
			...
			if( ( $this->Send['InvType'] != EcpayInvType::General ) && ( $this->Send['InvType'] != EcpayInvType::Special ) )
			{
		...
		if( $this->Invoice_Method == EcpayInvoiceMethod::INVOICE )
			...
			{
				if( ( $this->Send['vat'] != EcpayVatType::Yes ) && ( $this->Send['vat'] != EcpayVatType::No ) )
				{
		...
		// 檢查 B.延遲觸發開立發票
	        ...
	        {
	        	// 30.延遲註記 DelayFlag
			if( ( $this->Send['DelayFlag'] != EcpayDelayFlagType::Delay ) && ( $this->Send['DelayFlag'] != EcpayDelayFlagType::Trigger ) )
			{
			...
			if ( $this->Send['DelayFlag'] == EcpayDelayFlagType::Delay )
			{
			...
			if ($this->Send['DelayFlag'] == EcpayDelayFlagType::Trigger)
			{
		...
		// 檢查 B.延遲觸發開立發票、K.付款完成觸發或延遲開立發票
	        if( $this->Invoice_Method == EcpayInvoiceMethod::INVOICE_DELAY || $this->Invoice_Method == EcpayInvoiceMethod::INVOICE_TRIGGER)
	        {	
			// 33.交易單號 Tsr
			...
			// *2016-10-4 修改為僅允許 2.ECPAY
			if( $this->Send['PayType'] != EcpayPayTypeCategory::Ecpay )
			{
		...
		// 檢查 B.延遲觸發開立發票
			...
			switch ($this->Send['PayType'])
			{
              			...
              			case EcpayPayTypeCategory::Ecbank:
					$this->Send['PayAct'] = 'ECBANK' ;
				break;
                 		 
				// *若交易類別='2'時，則VAL = 'ECPAY'
              			case EcpayPayTypeCategory::Ecpay:
					$this->Send['PayAct'] = 'ECPAY' ;
				break;
              
              			// *若交易類別='3'時，則VAL = 'ALLPAY'
              			case EcpayPayTypeCategory::Allpay:
	        ...
	        if( $this->Invoice_Method == EcpayInvoiceMethod::ALLOWANCE || $this->Invoice_Method == EcpayInvoiceMethod::ALLOWANCE_VOID || $this->Invoice_Method == EcpayInvoiceMethod::ALLOWANCE_SEARCH || $this->Invoice_Method == EcpayInvoiceMethod::ALLOWANCE_VOID_SEARCH || $this->Invoice_Method == EcpayInvoiceMethod::INVOICE_NOTIFY )
	        {
	        	// 37.發票號碼 InvoiceNo
	        ...
	        {	
	        	...
	        	// *固定給定下述預設值
	        	if( ( $this->Send['AllowanceNotify'] != EcpayAllowanceNotifyType::Sms ) && ( $this->Send['AllowanceNotify'] != EcpayAllowanceNotifyType::Email ) && ( $this->Send['AllowanceNotify'] != EcpayAllowanceNotifyType::All ) && ( $this->Send['AllowanceNotify'] != EcpayAllowanceNotifyType::None ) )
			{
			...
			if(strlen($this->Send['NotifyMail']) > 0 )
	        	...
	        	if($this->Send['AllowanceNotify'] == EcpayAllowanceNotifyType::Email && strlen($this->Send['NotifyMail']) == 0 )
	        	{
	        	...
	        	if (strlen($this->Send['NotifyPhone']) > 20)
			{
	        	...
	        	if( $this->Send['AllowanceNotify'] == EcpayAllowanceNotifyType::Sms && strlen($this->Send['NotifyPhone']) == 0 )
	        	{
	        		array_push($arErrors, "40:NotifyPhone is required.");
	        	}
	        	
	        	// 39-40 通知電子信箱、通知手機號碼不能全為空值 (如果狀態為SMS 或 EMAIL)
	        	if(strlen($this->Send['NotifyPhone']) == 0 && strlen($this->Send['NotifyMail']) == 0 && ( $this->Send['AllowanceNotify'] == EcpayAllowanceNotifyType::Sms || $this->Send['AllowanceNotify'] == EcpayAllowanceNotifyType::Email ) )
	        	{
	        	...
	        	else
	        	{
		        	...
		        	if( $this->Send['AllowanceNotify'] == EcpayAllowanceNotifyType::All && ( strlen($this->Send['NotifyMail']) == 0 || strlen($this->Send['NotifyPhone']) == 0 ) )
		        	{
		        		array_push($arErrors, "39-40:NotifyMail And NotifyPhone is required.");
		        	}
		        	
		        	// *下述情況通知手機號碼與電子信箱為空值(通知類別為N-皆不通知)
		        	if($this->Send['AllowanceNotify'] == EcpayAllowanceNotifyType::None && ( strlen($this->Send['NotifyMail']) > 0 || strlen($this->Send['NotifyPhone']) > 0 ))
		        	{
		...
		// 檢查 D.發票作廢
	        ...
	        // 檢查 D.發票作廢、折讓作廢
	        if( $this->Invoice_Method == EcpayInvoiceMethod::INVOICE_VOID || $this->Invoice_Method == EcpayInvoiceMethod::ALLOWANCE_VOID )
	        {
			// 43.作廢原因 Reason
		...
		// 檢查 E.折讓作廢、H.查詢折讓明細 I.查詢折讓作廢明細、J.發送通知
	        if( $this->Invoice_Method == EcpayInvoiceMethod::ALLOWANCE_VOID || $this->Invoice_Method == EcpayInvoiceMethod::ALLOWANCE_SEARCH || $this->Invoice_Method == EcpayInvoiceMethod::ALLOWANCE_VOID_SEARCH || $this->Invoice_Method == EcpayInvoiceMethod::INVOICE_NOTIFY )
	        {
			// 44.折讓編號 AllowanceNo
		...
		// 檢查J.發送通知
			...
			if(strlen($this->Send['NotifyMail']) > 0 )
	        	...
	        	if( $this->Send['Notify'] == EcpayNotifyType::Email && strlen($this->Send['NotifyMail']) == 0 )
	        	{
	        		array_push($arErrors, "39:NotifyMail is required.");
	        	}
			// *UrlEncode
			...
			if(strlen($this->Send['Phone']) > 0 )
	        	...
	        	if( $this->Send['Notify'] == EcpayNotifyType::Sms && strlen($this->Send['Phone']) == 0 )
	        	{
	        	...
	        	{
	        		if( $this->Send['Notify'] == EcpayNotifyType::All && ( strlen($this->Send['NotifyMail']) == 0 || strlen($this->Send['Phone']) == 0 ) )
	        		{
	        	...
	        	// *固定給定下述預設值
	        	if( ($this->Send['Notify'] != EcpayNotifyType::Sms ) && ( $this->Send['Notify'] != EcpayNotifyType::Email ) && ( $this->Send['Notify'] != EcpayNotifyType::All ) )
	        	{
			...
			// *固定給定下述預設值
			if( ( $this->Send['InvoiceTag'] != EcpayInvoiceTagType::Invoice ) && ( $this->Send['InvoiceTag'] != EcpayInvoiceTagType::Invoice_Void ) && ( $this->Send['InvoiceTag'] != EcpayInvoiceTagType::Allowance ) && ( $this->Send['InvoiceTag'] != EcpayInvoiceTagType::Allowance_Void ) && ( $this->Send['InvoiceTag'] != EcpayInvoiceTagType::Invoice_Winning ) )
	        	{
			...
			// *固定給定下述預設值
			if( ( $this->Send['Notified'] != EcpayNotifiedType::Customer ) && ( $this->Send['Notified'] != EcpayNotifiedType::vendor ) && ( $this->Send['Notified'] != EcpayNotifiedType::All ) )
	        	{
		...
		// 檢查M.愛心碼驗證
			...
			// 51.LoveCode愛心碼
	        	...
	        	// *必填 3-7碼
	        	if( strlen($this->Send['LoveCode']) > 7 )
	        	{
	...
	*/
	function GenerateCheckMacValue($arParameters)
	{
		...
		if(isset($arParameters))
		{
			...
			uksort($arParameters, array('EcpayInvoice','merchantSort'));
